...
*/
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        > {
            ...
            let codec = tonic::codec::ProstCodec::default();
            let path = http::uri::PathAndQuery::from_static(
                "/filescale.v1.RegisterService/Register",
            );
            let mut req = request.into_request();
            req.extensions_mut()
                .insert(GrpcMethod::new("filescale.v1.RegisterService", "Register"));
...
pub mod register_service_server {
    ...
    #[async_trait]
    pub trait RegisterService: std::marker::Send + std::marker::Sync + 'static {
        async fn register(
            &self,
...
*/
    ...
    impl<T> RegisterServiceServer<T> {
        pub fn new(inner: T) -> Self {
            Self::from_arc(Arc::new(inner))
        }
        pub fn from_arc(inner: Arc<T>) -> Self {
                ...
                "/filescale.v1.RegisterService/Register" => {
                    ...
                    for RegisterSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as RegisterService>::register(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    impl<T> Clone for RegisterServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    /// Generated gRPC service name
    pub const SERVICE_NAME: &str = "filescale.v1.RegisterService";
    impl<T> tonic::server::NamedService for RegisterServiceServer<T> {
        const NAME: &'static str = SERVICE_NAME;
    }
}
